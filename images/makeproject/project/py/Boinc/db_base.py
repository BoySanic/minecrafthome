...
def _commit_object(tablename, paramdict, id=None):
    ...
    for key in paramdict.keys():
        ...
        if value is None:
            ...
            equalcommands.append("%s='%s'"%(key,dbconnection.escape_string(str(value)).decode()))
    if id is None:
...
def _select_object(table, searchdict, extra_args="", extra_params=[], select_what=None):
    ...
    for (key,value) in searchdict.items():
        if value is None:
            value = ''
        escaped_value = dbconnection.escape_string(str(value)).decode()
        if key == 'text':
...
class DatabaseTable:
    ...
    def _valid_query_keys(self):
        return list(self.columns.keys())+list(self.extra_columns.keys())+['_join','_extra_params']
